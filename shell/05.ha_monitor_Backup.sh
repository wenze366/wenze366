#!/bin/bash
# ===============================================================================
# 脚本名称: 05.ha_monitor_Backup.sh
# 脚本用途: GBase 监控系统备HA调度器
# 部署位置: 172.16.213.105 (Backup HA 监控机)
#
# 功能说明:
...
#
# 执行方式:
#   手动执行: bash /path/to/05.ha_monitor_Backup.sh
#   定时任务: */5 * * * * /path/to/05.ha_monitor_Backup.sh >> /var/log/ha_105.log 2>&1
#
# 依赖项:
...
echo "--- $(date '+%Y-%m-%d %H:%M:%S') - Backup HA 状态检查 (105) ---"
...
# --- 决策逻辑 (172.16.213.105 规则) ---

SHOULD_RUN_MONITOR="false"
ACTION_REASON="Backup 监控机 (105) 默认停止执行"

# 规则 1: 必须满足，三台机器，monitor.pid 内容为都为 Backup ID:2000，开始执行
if [ "$backup_count" -eq 3 ]; then
    SHOULD_RUN_MONITOR="true"
    ACTION_REASON="检测到所有节点状态都为 Backup ID:2000，开始执行"

# 规则 2: 必须满足 三台机器，都无法 ssh 获取信息，开始执行
elif [ "$unreachable_count" -eq 3 ]; then
    SHOULD_RUN_MONITOR="true"
    ACTION_REASON="检测到所有节点都 Unreachable (SSH 失败)，开始执行"

# 规则 3: 如检测到 三台机器，monitor.pid 内容为都为 Primary ID:1000，停止执行
elif [ "$primary_count" -eq 3 ]; then
    SHOULD_RUN_MONITOR="false"
    ACTION_REASON="检测到所有节点状态都为 Primary ID:1000，停止执行"
fi
...
echo "--- $(date '+%Y-%m-%d %H:%M:%S') - Backup HA 状态检查结束 ---"
exit 0
